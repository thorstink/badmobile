...
createLSM9DS1Observable(const nlohmann::json &settings) {
  return rxcpp::observable<>::create<imu_t>(
      [=, imu_settings = settings["robot"]["hardware"]["imu"]](
          rxcpp::subscriber<imu_t> s) {
        ...
        const auto write_ag_u8 = [=](int g_spi_handle, char address,
                                     char value) {
        ...
        const auto init_lsm9ds1 = [=](int g_spi_handle) {
...

          {
            // set range 2G
            uint8_t reg =
                read_ag_u8(g_spi_handle, LSM9DS1_REGISTER_CTRL_REG6_XL);
            reg &= ~(0b00011000);
          ...
          }
          {
            // set dpi 245
            uint8_t reg =
                read_ag_u8(g_spi_handle, LSM9DS1_REGISTER_CTRL_REG1_G);
            reg &= ~(0b00110000);
            reg |= LSM9DS1_ACCELRANGE_2G;
